...
async function joinOrCreateRoom(isPrivate, roomCode = null) {
  ...
  try {
    const result = await DB.findOrCreateIsimSehirRoom(isPrivate, roomCode);
    ...
    unsubscribeListener = DB.listenToIsimSehirRoom(currentRoomId, (roomData) => {
       if (roomData.status === "playing") {
          window.location.href = `./isim-sehir.html?roomId=${currentRoomId}`;
       }
    });
